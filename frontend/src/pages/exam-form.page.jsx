...
import { createExam, getExam, updateExam } from "../services/exam.service";
...
const HISTORY_TAB = "exam";
const ENTITY_TYPE = "EXAM";
const ATTACHMENT_CONTEXT = "EXAM_ATTACHMENT";
...
function emptyExamForm() {
  return {
    ...
    indicaciones: EMPTY_EDITOR_HTML,
    monto_total: "",
    moneda: "CLP",
    ...
    <section className="clinical-form-section clinical-form-section-separated">
      <div className="clinical-section-heading">
        <div>
          ...
          <p>Registra la obligación financiera asociada al examen cuando corresponda.</p>
        </div>
      </div>
      ...
      {payableLocked ? (
        <p className="clinical-field-help">
          La cuenta por pagar posee pagos o movimientos asociados y sus datos financieros no pueden modificarse desde este examen.
        </p>
      ) : null}
...
export default function ExamFormPage() {
    ...
    try {
      ...
      setFilesError(
        error instanceof Error
          ? error.message
          : "No se pudieron cargar los adjuntos del examen.",
      );
    } finally {
  ...
  useEffect(() => {
    ...
    async function loadPage() {
      ...
      try {
        const [animalResponse, clinicsResponse, examResponse] = await Promise.all([
          getAnimal(animalId),
          getVetClinics({ activo: true }),
          isEdit ? getExam(examId) : Promise.resolve(null),
        ]);
        ...
        setRecord(examResponse);

        if (examResponse) {
          const currentVeterinarian = examResponse.veterinarianId
            ? {
                id: examResponse.veterinarianId,
                nombreCompleto: examResponse.veterinarianNombre || "Veterinario histórico",
              }
            : null;
          setHistoricalVeterinarian(currentVeterinarian);
          setForm({
            fecha_solicitud: examResponse.fechaSolicitud || "",
            nombre_examen: examResponse.nombreExamen || "",
            motivo: toEditorHtml(examResponse.motivo),
            peso: examResponse.peso ?? "",
            temperatura: examResponse.temperatura ?? "",
            fecha_entrega_resultado: examResponse.fechaEntregaResultado || "",
            diagnostico: toEditorHtml(examResponse.diagnostico),
            indicaciones: toEditorHtml(examResponse.indicaciones),
            monto_total: examResponse.montoTotal ?? examResponse.precio ?? "",
            moneda: examResponse.moneda || "CLP",
            genera_cuenta_por_pagar: Boolean(examResponse.generaCuentaPorPagar),
            fecha_vencimiento_pago: examResponse.fechaVencimientoPago || "",
            observacion_financiera: examResponse.observacionFinanciera || "",
            veterinarian_id: examResponse.veterinarianId
              ? String(examResponse.veterinarianId)
              : "",
            clinic_id: examResponse.clinicId ? String(examResponse.clinicId) : "",
          });

          await Promise.all([
            loadVeterinariansForClinic(examResponse.clinicId),
            loadExistingFiles(examResponse.id),
          ]);
        }
        if (!examResponse) {
        ...
        setLoadError(
          error instanceof Error
            ? error.message
            : "No se pudo cargar el formulario del examen.",
        );
      } finally {
        if (isMounted) {
  ...
  }, [animalId, examId, isEdit, loadExistingFiles, loadVeterinariansForClinic]);

  const pageTitle = isEdit ? "Editar examen" : "Nuevo examen";
  ...
  const isFormValid = useMemo(() => {
    return Boolean(
      ...
      && form.nombre_examen.trim()
      && richTextToPlainText(form.motivo)
      && form.clinic_id,
    );
  }, [form]);
  ...
  const handleSubmit = async (event) => {
    ...
    if (!isFormValid || isSubmitting) {
      setFormError("Completa la fecha, el nombre, el motivo y la clínica del examen.");
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        fecha_entrega_resultado: form.fecha_entrega_resultado || null,
        diagnostico: normalizeRichTextForPayload(form.diagnostico),
        indicaciones: normalizeRichTextForPayload(form.indicaciones),
      ...
      if (!isEdit && pendingFiles.length > 0 && canUploadClinicalFiles) {
        ...
        if (uploadResult.failed.length > 0) {
          navigate(`/rescatados/${animalId}/examenes/${savedRecord.id}/editar`, {
            replace: true,
            state: {
              attachmentWarning: `El examen se creó correctamente, pero ${uploadResult.failed.length} adjunto(s) no pudieron subirse.`,
            },
          });
          return;
        }
      ...
      setFormError(
        error instanceof Error ? error.message : "No se pudo guardar el examen.",
      );
    } finally {
  ...
  const payableLocked = Boolean(
    record?.payableAccount?.pago_cuenta_por_pagar_id
      || record?.payableAccount?.transaccion_id,
  );
      ...
      {!loadError && !isLoading ? (
        ...
        >
          ...
          <section className="clinical-form-section">
            <div className="clinical-section-heading">
              <div>
                ...
                <p>Completa los antecedentes principales del examen veterinario.</p>
              </div>
            </div>

            <div className="clinical-form-grid">
              <label className="clinical-field">
                <span className="clinical-field-label">Fecha del examen *</span>
                <input
                  type="date"
                  value={form.fecha_solicitud}
                  onChange={(event) => handleChange("fecha_solicitud", event.target.value)}
                  required
                />
              </label>

              <label className="clinical-field">
                <span className="clinical-field-label">Nombre del examen *</span>
                <input
                  type="text"
                  value={form.nombre_examen}
                  onChange={(event) => handleChange("nombre_examen", event.target.value)}
                  required
                />
              </label>
              ...
              <label className="clinical-field">
                <span className="clinical-field-label">Entrega de resultado</span>
                <input
                  type="date"
                  value={form.fecha_entrega_resultado}
                  onChange={(event) => handleChange("fecha_entrega_resultado", event.target.value)}
                />
              </label>
            </div>
          </section>
          ...
          <section className="clinical-form-section clinical-form-section-separated">
            <div className="clinical-section-heading">
              <div>
                <h2>Resultado e indicaciones</h2>
                <p>Registra los antecedentes posteriores cuando estén disponibles.</p>
              </div>
            </div>
            ...
            <div className="clinical-form-grid">
              ...
              <ClinicalRichTextField
                ...
                disabled={isSubmitting}
                onChange={(value) => handleChange("indicaciones", value)}
              />
            </div>
          </section>
