...
import { createProcedure, getProcedure, updateProcedure } from "../services/procedure.service";
...
const HISTORY_TAB = "procedure";
const ENTITY_TYPE = "PROCEDURE";
const ATTACHMENT_CONTEXT = "PROCEDURE_ATTACHMENT";
...
function ProcedurePayableFields({
  form,
  onChange,
  visible,
  canEdit,
    ...
    <section className="clinical-form-section clinical-form-section-separated">
      <div className="clinical-section-heading">
        <div>
          ...
          <p>Registra la obligación financiera asociada el procedimiento cuando corresponda.</p>
        </div>
      </div>
      ...
      {payableLocked ? (
        <p className="clinical-field-help">
          La cuenta por pagar posee pagos o movimientos asociados y sus datos financieros no pueden modificarse desde este procedimiento.
        </p>
      ) : null}
...
export default function ProcedureFormPage() {
  ...
  useEffect(() => {
    ...
    async function loadPage() {
      ...
      try {
        const [animalResponse, clinicsResponse, procedureResponse] = await Promise.all([
          getAnimal(animalId),
          getVetClinics({ activo: true }),
          isEdit ? getProcedure(procedureId) : Promise.resolve(null),
        ]);
        ...
        setRecord(procedureResponse);

        if (procedureResponse) {
          const currentVeterinarian = procedureResponse.veterinarianId
            ? {
                id: procedureResponse.veterinarianId,
                nombreCompleto: procedureResponse.veterinarianNombre || "Veterinario histórico",
              }
            : null;
          setHistoricalVeterinarian(currentVeterinarian);
          setForm({
            fecha_procedimiento: procedureResponse.fechaProcedimiento || "",
            tipo: procedureResponse.tipo || "",
            motivo: toEditorHtml(procedureResponse.motivo),
            observaciones: toEditorHtml(procedureResponse.observaciones),
            farmacos_recetados: toEditorHtml(procedureResponse.farmacosRecetados),
            indicaciones: toEditorHtml(procedureResponse.indicaciones),
            monto_total: procedureResponse.montoTotal ?? procedureResponse.precio ?? "",
            moneda: procedureResponse.moneda || "CLP",
            genera_cuenta_por_pagar: Boolean(procedureResponse.generaCuentaPorPagar),
            fecha_vencimiento_pago: procedureResponse.fechaVencimientoPago || "",
            observacion_financiera: procedureResponse.observacionFinanciera || "",
            veterinarian_id: procedureResponse.veterinarianId ? String(procedureResponse.veterinarianId) : "",
            clinic_id: procedureResponse.clinicId ? String(procedureResponse.clinicId) : "",
          });

          await Promise.all([
            loadVeterinariansForClinic(procedureResponse.clinicId),
            loadExistingFiles(procedureResponse.id),
          ]);
        }
        if (!procedureResponse) {
        ...
        setLoadError(
          error instanceof Error
            ? error.message
            : "No se pudo cargar el formulario del procedimiento.",
        );
      } finally {
        if (isMounted) {
  ...
  }, [animalId, isEdit, loadExistingFiles, loadVeterinariansForClinic, procedureId]);

  const pageTitle = isEdit ? "Editar procedimiento" : "Nuevo procedimiento";
  ...
  const isFormValid = useMemo(() => {
    return Boolean(
      form.fecha_procedimiento && form.tipo.trim() && richTextToPlainText(form.motivo) && form.clinic_id,
    );
  }, [form]);
  ...
  const handleSubmit = async (event) => {
    ...
    if (!isFormValid || isSubmitting) {
      setFormError("Completa la fecha, el tipo, el motivo y la clínica del procedimiento.");
      return;
    }
    ...
    try {
      ...
      if (!isEdit && pendingFiles.length > 0 && canUploadClinicalFiles) {
        ...
        if (uploadResult.failed.length > 0) {
          navigate(`/rescatados/${animalId}/procedimientos/${savedRecord.id}/editar`, {
            replace: true,
            state: {
              attachmentWarning: `El procedimiento se creó correctamente, pero ${uploadResult.failed.length} adjunto(s) no pudieron subirse.`,
            },
          });
          return;
        }
      ...
      {!loadError && !isLoading ? (
        ...
        >
          ...
          <section className="clinical-form-section">
            <div className="clinical-section-heading">
              <div>
                ...
                <p>Completa los antecedentes principales del procedimiento veterinario.</p>
              </div>
            </div>

            <div className="clinical-form-grid">
              <label className="clinical-field">
                <span className="clinical-field-label">Fecha del procedimiento *</span>
                <input
                  type="date"
                  value={form.fecha_procedimiento}
                  onChange={(event) => handleChange("fecha_procedimiento", event.target.value)}
                  required
                />
              </label>

              <label className="clinical-field">
                <span className="clinical-field-label">Tipo *</span>
                <input
                  type="text"
                  value={form.tipo}
          ...
          <section className="clinical-form-section clinical-form-section-separated">
            <div className="clinical-section-heading">
              <div>
                <h2>Observaciones e indicaciones</h2>
