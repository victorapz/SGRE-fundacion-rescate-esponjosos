...
const { default: accountingReportRoutes } = await import(
  new URL("./accounting_report.routes.js?payables-rbac-test", import.meta.url)
);
...
function getPayablesPreviewHandlers() {
  ...
  const previewRouteLayer = accountingReportRoutes.stack.find(
    (layer) => layer.route?.path === "/payables",
  );

  if (!authenticateLayer || !previewRouteLayer) {
    throw new Error("No fue posible localizar la ruta del preview de cuentas por pagar.");
  }
...
function getPayablesExportHandlers() {
  const authenticateLayer = accountingReportRoutes.stack.find((layer) => !layer.route);
  const exportRouteLayer = accountingReportRoutes.stack.find(
    (layer) => layer.route?.path === "/payables/export",
  );

  if (!authenticateLayer || !exportRouteLayer) {
    throw new Error("No fue posible localizar la ruta de exportacion de cuentas por pagar.");
  }
...
test("preview de payables responde 401 sin usuario autenticado", async () => {
  const { authenticate, authorize, controller } = getPayablesPreviewHandlers();
  const req = { query: {} };
...
test("preview de payables responde 403 sin permiso", async () => {
  const { authenticate, authorize, controller } = getPayablesPreviewHandlers();
  const req = {
    authenticatedUser: { id_usuario: 91 },
    query: {},
  };
  const res = buildMockResponse();
  let controllerReached = false;

  await withFakeUserRepository(buildUserWithPermissions(["accounting:transaction:read"]), async () => {
...
test("preview de payables supera auth y RBAC con permiso valido antes del controller", async () => {
  const { authenticate, authorize, controller } = getPayablesPreviewHandlers();
  const req = {
    authenticatedUser: { id_usuario: 91 },
    query: { proveedor_id: "-1" },
  };
  const res = buildMockResponse();
  let controllerReached = false;
...
test("export de payables responde 403 si falta permiso de exportacion", async () => {
  const { authenticate, authorizeAny, authorizeAll } = getPayablesExportHandlers();
  const req = {
    authenticatedUser: { id_usuario: 91 },
    query: { format: "xlsx" },
  };
...
test("export de payables supera auth y permisos antes del controller", async () => {
  const { authenticate, authorizeAny, authorizeAll, controller } = getPayablesExportHandlers();
  const req = {
    authenticatedUser: { id_usuario: 91 },
    query: {},
  };
  ...
  await withFakeUserRepository(
    buildUserWithPermissions(["accounting:payable:read", "accounting:report:export"]),
    async () => {
      await authenticate(req, res, async () => {
        await authorizeAny(req, res, async () => {
          await authorizeAll(req, res, () => {
